...
$query = "Select ID,GROUP_NAME From  content_group  Where GROUP_TYPE= '6' Order by ID";
...
if (strtoupper($_SERVER['REQUEST_METHOD'])=="POST"  && isset($_POST)) {
	...
	//		$txtarea = htmlspecialchars( $_POST['editor1'] ) ;
		...
		if ($insert) {
			AlertMessage("บันทึกข้อมูล demo faris เรียบร้อยแล้วค่ะ ...","Administrator/vfarisdemo.php");
			exit;
		} 
		else {
...
*/
		...
		if ($rows_content >0) {
			...
			AlertMessage("ไม่พบข้อมูล demo faris กรุณาลองใหม่อีกครั้ง !!","vfarisdemo.php");
			exit;
		}
		$Mode="Update";
...
<table width="900"  border="0" align="center" cellpadding="0" cellspacing="0" style="border:solid #6497D6 1px" bgcolor="#009999">
  <tr>
    <td width="806" align="left"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <table width="228" border="0" cellpadding="0" cellspacing="0" >
            <tr>
              <th align="left" valign="top" >สร้าง/เพิ่ม File DEMO : FARIS By Naris</th>
              </tr>
          </table></td>
  ...
  <tr>
    <td bgcolor="#F1F3F5"><form id="form1" name="form1" method="post" action="" onsubmit="return check();">
        ...
        <table width="100%" border="0" cellspacing="1" cellpadding="2">
		  ...
		  ?>
            </select>              <input name="ID" type="hidden" id="ID" value="<?php echo $id; ?>" />
            *เลือก Faris</td>
          </tr>
          <tr>
...
function convertRGBToHex( cssStyle )
{
	return cssStyle.replace( /(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function( match, red, green, blue )
        ...
        <center>
          ...
          <input type="button" name="button" id="button" value="Cancel"  onclick="window.location='administrator/vfarisdemo.php'"/>
