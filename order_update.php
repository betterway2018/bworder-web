...
if ($totalRows_order==0) {
			// GENERATE ORDER_NO
			$sql ="SELECT MAX(ORDER_NO) as MAXNO FROM ORDER_HEADER WHERE ORDCAMP = '$campaign' and dist = '$dist' and mslno = '$mslno' and chkdgt = '$chkdgt'";
...
$sql_del="UPDATE ORDER_DETAIL SET DELFLAG = 'T' , listno=listno*(-1) WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
...
//echo $sql_del;
	...
	chkdetail();
 /*
	$query_campaign = "SELECT count(*)  totaldetail from order_detail where ORDER_NO = '$order_no' and ORDCAMP = '$campaign' ";
	...
	if($totaldetail!=$i_item){
			...
			$sql_del="UPDATE ORDER_DETAIL SET DELFLAG = 'N' , listno=listno*(-1) WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
