...
$query_order .= "FROM order_headertemp WHERE DWNFLAG='N' AND DELFLAG='N' ";
...
if ($totalRows_order==0) {
			// GENERATE ORDER_NO
			$sql ="SELECT MAX(ORDER_NO) as MAXNO FROM order_headertemp WHERE ORDCAMP = '$campaign' and dist = '$dist' and mslno = '$mslno' and chkdgt = '$chkdgt'";
...
$sql_del="UPDATE order_detailtemp SET DELFLAG = 'T' , listno=listno*(-1) WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
$sql_del .=" AND DIST='$dist' AND MSLNO=$mslno AND CHKDGT =$chkdgt and listno >0";
...
for ($i=1;$i<=$totalItems;$i++){
			if ($_POST['txtcode_'.$i]!="" && $_POST['txtqty_'.$i] !="") {
					...
					$query ="INSERT INTO order_detailtemp (";
					...
					$query .="$order_no,$campaign,$current_date,'$current_time',$curcamp,'$dist',$mslno,$chkdgt,";
					...
					if (!$result_line) {
						mysql_query("ROLLBACK");
//						die("Error insert: ".$query.mysql_error());
...
} catch (Exception $e) { echo "asdf";} 
//echo $sql_del;

	chkdetail();
 /*
	$query_campaign = "SELECT count(*)  totaldetail from order_detailtemp where ORDER_NO = '$order_no' and ORDCAMP = '$campaign' ";
	...
	if($totaldetail!=$i_item){
			$sql_del="DELETE FROM order_detailtemp  WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
			$sql_del .=" AND DIST='$dist' AND MSLNO=$mslno AND CHKDGT =$chkdgt AND DELFLAG='N' AND listno > 0 ";
			$result_del=mysql_query($sql_del,$bwc_orders);
			if (!result_del) {
				mysql_query("ROLLBACK");
				die('Error 0 :' . mysql_error());
			...
			$sql_del="UPDATE order_detailtemp SET DELFLAG = 'N' , listno=listno*(-1) WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
...
//######################################################################################
// INSERT  order_headertemp
//######################################################################################
if ($totalRows_order==0) {
			//INSERT HEADER
			$sql = "INSERT INTO order_headertemp (ORDER_NO,ORDCAMP,ORDDATE,ORDTIME,CURCAMP,DIST,MSLNO,CHKDGT,NAME,
		...
		// UPDATE DATA
			$sql ="UPDATE order_headertemp SET  
...
function chkdetail(){
	...
	global $bwc_orders;
	global $i_item;
	//echo "มมมมมมมมมมมมมมม". $database_bwc_orders."<br>";

	$query_campaign = "SELECT count(*)  totaldetail from order_detailtemp where ORDER_NO = '$order_no' and ORDCAMP = '$campaign' ";
	$query_campaign .=" AND DELFLAG = 'N' AND listno > 0 AND DIST='$dist' AND MSLNO='$mslno' AND CHKDGT ='$chkdgt' ";
	//echo "อออออออออออออออ". $query_campaign."<br>";
	...
	if($totaldetail!=$i_item || ($totaldetail==0 && $i_item==0)){
			...
			$sql_del="UPDATE order_detailtemp SET DELFLAG = 'N' , listno=listno*(-1) WHERE ORDER_NO = $order_no AND ORDCAMP=$campaign ";
			$sql_del .=" AND DIST='$dist' AND MSLNO='$mslno' AND CHKDGT ='$chkdgt' AND DELFLAG='T' AND listno < 0 ";
			echo $sql_del."<br>";
			$result_del=mysql_query($sql_del,$bwc_orders);
			if (!result_del) {
				mysql_query("ROLLBACK");
