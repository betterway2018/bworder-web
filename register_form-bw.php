...
<script language = "JavaScript">
		...
		{
			form_login.txtamphur.length = 0
			form_login.txttumbol.length = 0
			//*** Insert null Default Value ***//
			var myOption = new Option('','')  
			form_login.txtamphur.options[form_login.txtamphur.length]= myOption
			
			<?php
			$intRows = 0;
			$strSQL = "SELECT * FROM db_amphur ORDER BY AMPHUR_CODE ASC ";
			$objQuery = mysql_query($strSQL, $bwc_orders) or die ("Error Query [".$strSQL."]");
			$intRows = 0;
			...
			?>
				...
				strGroup = <?php echo $objResult["PROVINCE_CODE"];?>;
				strValue = "<?php echo $objResult["AMPHUR_CODE"];?>";
				strItem = "<?php echo $objResult["AMPHUR_LNAME"];?>";
...
//**** List District (Start) *** ตำบล//
		function Listdistrict(SelectValue)
		{
			...
			//*** Insert null Default Value ***//
			var myOption = new Option('','')  
			form_login.txttumbol.options[form_login.txttumbol.length]= myOption
			
			<?php
			$intRows = 0;
			$strSQL = "SELECT * FROM db_tumbon ORDER BY TUMBON_CODE  ASC ";
			$objQuery = mysql_query($strSQL, $bwc_orders) or die ("Error Query [".$strSQL."]");
			$intRows = 0;
			while($objResult = mysql_fetch_array($objQuery))
			{
			...
			?>
				...
				strGroup = <?php echo $objResult["AMPHUR_CODE"];?>;
				strValue = "<?php echo $objResult["TUMBON_CODE"];?>";
				strItem = "<?php echo $objResult["TUMBON_LNAME"];?>";
				mySubList[x,0] = strItem;
				mySubList[x,1] = strGroup;
				mySubList[x,2] = strValue;
  ...
  <fieldset style="width:600px">
    <table width="582" border="0" cellspacing="1" cellpadding="3" >
      ...
      <tr>
        ...
        <td align="left"><select id="txtprovince2" name="txtprovince2" style="width:370px" onchange = "ListAmphur(this.value)">
          <option selected="selected" value=""></option>
          <?php
	$strSQL = "SELECT * FROM db_province ORDER BY PROVINCE_CODE ASC ";
	$objQuery = mysql_query($strSQL, $bwc_orders) or die ("Error Query [".$strSQL."]");
	while($objResult = mysql_fetch_array($objQuery))
	{
	?>
          <option value="<?php echo $objResult["PROVINCE_CODE"];?>"><?php echo $objResult["PROVINCE_LNAME"];?></option>
          <?php
	}
	?>
        </select></td>
      ...
      <tr>
        ...
        <td align="left"><select id="txtamphur" name="txtamphur" style="width:370px" onchange = "Listdistrict(this.value)">
        </select></td>
        <td align="left">&nbsp;</td>
      </tr>
      <tr>
        <td align="right">Thambon/ ตำบล-แขวง :</td>
        <td align="left"><select id="txttumbol" name="txttumbol" style="width:370px">
        </select></td>
        <td align="left">&nbsp;</td>
      </tr>
      <tr>
