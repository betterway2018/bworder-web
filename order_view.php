...
if($rep_seq <>  "")
{
  $rep_seq1 =  "rep_seq = $rep_seq and ";

}
else
{

$rep_seq1="dist='$dist' and mslno='$mslno' and chkdgt='$chkdgt' and ";
...
if($dist == "0999")
{
        ...
        $query_order = " SELECT *  FROM order_header   WHERE  dist='$dist' and mslno='$mslno' and chkdgt='$chkdgt'  and order_no='$po'  and ordcamp='$camp' AND DELFLAG='N'";
        ...
        $totalRows_order = mysql_num_rows($order);
         ...
         mysql_select_db($database_bwc_orders, $bwc_orders);
        $query_order_line = "SELECT * FROM order_detail  WHERE dist='$dist' and mslno='$mslno' and chkdgt='$chkdgt'  and order_no='$po' and ordcamp='$camp' order by  ListNO";
...
{ 
        mysql_select_db($database_bwc_orders, $bwc_orders);
        $query_order = " SELECT *  FROM order_header    WHERE dist='$dist' and mslno='$mslno' and chkdgt='$chkdgt'   and order_no='$po' AND DELFLAG='N' and ordcamp='$camp' ";
        $order = mysql_query($query_order, $bwc_orders) or die(mysql_error());
         //echo $query_order; exit;
        $row_order = mysql_fetch_assoc($order);
        $totalRows_order = mysql_num_rows($order);
         ...
         mysql_select_db($database_bwc_orders, $bwc_orders);
        $query_order_line = "SELECT * FROM order_detail  WHERE dist='$dist' and mslno='$mslno' and chkdgt='$chkdgt' and ordcamp='$camp'  AND DELFLAG='N' order by  ListNO";
		//echo $query_order_line; exit;
        $order_line = mysql_query($query_order_line, $bwc_orders) or die(mysql_error());
        $row_order_line = mysql_fetch_assoc($order_line);
        $totalRows_order_line = mysql_num_rows($order_line);
  ...
  <tr>
    ...
    <table width="740" border="0" align="center" cellpadding="0" cellspacing="1"  style="border:#ec008c solid 1px;margin-top:3px">
      <tr style="background-image:url(image_icon/bg_cell.gif);background-position:top;background-repeat:repeat-x;color:#226543;font-weight:bold">
		...
		<!--<td width="86" align="right" nowrap="nowrap"><strong class="content_header3">สถานะสินค้า</strong></td>-->
      </tr>
...
do { 
	...
	$DELFLAG=$row_order_line['DELFLAG'];
	$AMOUNT=$row_order_line['AMOUNT'];
	/*
	switch ($DELFLAG) {
    case "N": $img="<img src='img/OK.png' />";
        break;
    case "Y": $img="<img src='img/Remove.png' />"; $AMOUNT=0;
	 break;
    default:
        echo $img="<img src='img/Empty.png' />";
}
*/
$amt=$amt+$AMOUNT;
?>
      <tr bgcolor="#FFF0FE" style=" height:26px;">
        ...
        <td align="right" class="line_bottom" style="padding-right:3px"><?php echo  $i; ?></td>
        ...
        <td align="right" class="line_bottom"><?php echo  " ". intToMoney($row_order_line['AMOUNT']); ?>&nbsp;</td>
		 <td align="center" class="line_bottom"><?php echo " ".$img ;?>&nbsp;</td>
      </tr>
      <?php 
  	$i=$i+1;
    ...
    <table width="740"  border="0" align="center" cellpadding="2" cellspacing="1">
      <tr>
        ...
        <td width="85" align="right"  style="padding-right:6px"><strong><?php echo intToMoney($amt); ?></strong></td>
      </tr>
    </table>
    <br />
    <br />
...
Betterway (Thailand) Co.,Ltd. ทะเบียนพาณิชย์เลขที่ : 010553102764...<br>
<br><!--?  //echo "Redirect from " . $_SESSION['website']; ?--></font> 
</center>
