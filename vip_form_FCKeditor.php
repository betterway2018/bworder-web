...
if (strtoupper($_SERVER['REQUEST_METHOD'])=="POST"  && isset($_POST)) {
	...
	//		$txtarea = htmlspecialchars( $_POST['editor1'] ) ;
		...
		if ($insert) {
			AlertMessage("บันทึกข้อมูล NEWS AND EVENT เรียบร้อยแล้วค่ะ ...","Administrator/vip.php");
			exit;
		} 
		else {
...
*/
		...
		if ($rows_content >0) {
			...
			AlertMessage("ไม่พบข้อมูล กรุณาลองใหม่อีกครั้ง !!","vip.php");
			exit;
		}
		$Mode="Update";
...
<table width="900"  border="0" align="center" cellpadding="0" cellspacing="0" style="border:solid #6497D6 1px" bgcolor="#9999FF">
  <tr>
    <td width="806" align="left"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <table width="228" border="0" cellpadding="0" cellspacing="0" >
            <tr>
              <th align="left" valign="top" >สร้าง/เพิ่ม File NEWS AND EVENTS</th>
              </tr>
          </table></td>
...
function convertRGBToHex( cssStyle )
{
	return cssStyle.replace( /(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function( match, red, green, blue )
        ...
        <center>
          ...
          <input type="button" name="button" id="button" value="Cancel"  onclick="window.location='administrator/vip.php'"/>
