...
$query = "SELECT * FROM WEBBOARD_GROUP ORDER BY  Order_seq asc";
...
<script language="JavaScript">
	   ...
	   function doCallAjax(Sort,page,dist,row_index) {
		  ...
		  if (!HttPRequest) {
			...
			//dist = '999';
			var pmeters =  'Sort='+Sort+'&page='+page+'&dist='+dist+'&mslno='+mslno+'&chkdgt='+chkdgt+'&datefrom='+datefrom+'&dateto='+dateto+'&email='+email+"&fselect="+fselect+"&sel_group="+sel_group;
...
//			var pmeters =  'Sort='+Sort+'&page='+page+'&dist='+dist+'&mslno='+mslno+'&chkdgt='+chkdgt+'&mslname='+mslname +'&datefrom='+datefrom+'&dateto='+dateto+'&email='+email;
		  ...
		  if (!HttPRequest) {
			...
			var url = 'data_contactus_Ajax.php';
			var pmeters =  'Sort=&page=&dist='+dist+'&mslno='+mslno+'&chkdgt='+chkdgt+'&datefrom='+datefrom+'&dateto='+dateto+'&email='+email+"&row_index=" + row_index + "&tMode=" + Mode+"&fselect="+fselect+"&sel_group="+sel_group;
...
<strong class="text_heading">::   ติดต่อสอบถาม ถาม-ตอบ :โดยคอลเซ็นเตอร์ ::</strong>
	...
	new tcal ({
        ...
        <tr>
          <td align="right">&nbsp;</td>
          <td colspan="2"><font color="red" >กรุณาระบุเลือกวันที่ท่านต้องการท่านค้นหา</font></td>
        </tr>
        <tr>
          <td align="right">ระบุรหัสสมาชิก :</td>
...
ค้นหาด้วยรหัสสมาชิก</font></td>
        </tr>
        <tr>
          <td align="right">e-mail :</td>
          <td colspan="2"><input name="email" type="text" id="email" size="48" /> 
           <font color="red" size="-1" > ค้นหาด้วยอีเมลล์</font></td>
        </tr>
        <tr>
          <td align="right">ค้นหาด้วย :</td>
          <td colspan="2">
          	<select name="fselect" id="fselect" onchange="change_select();">
            <option value="0">รายการที่ยังไม่ตอบคำถาม</option>
            <option value="1">รายการที่ตอบคำถามแล้ว</option>
