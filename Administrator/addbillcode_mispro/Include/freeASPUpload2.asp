...
Class FreeASPUpload
	...
	Public Sub SaveAs(path,sfile)
		...
		streamFile.Type = 1
		streamFile.Open
		StreamRequest.Position=UPloadedFiles.Items(sfile).Start
		StreamRequest.CopyTo streamFile,UPloadedFiles.Items(sfile).Length
		streamFile.SaveToFile path & sfile, 2
		streamFile.close
		Set streamFile = Nothing
		fileItem.Path = path & sfile
		...
		For Each fileItem In UploadedFiles.Items
			...
			StreamRequest.Position=fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			streamFile.SaveToFile path & file, 2
			streamFile.close
			Set streamFile = Nothing
			fileItem.Path = path & file
		 Next
	End Sub
